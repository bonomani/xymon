		       ...
		       FILE *output)
{
	...
	if (rrd && graph) {
		int may_have_rrd = 1;
		int devmon_markers_found;

		/*
		 * Scan the payload for embedded DEVMON graph markers regardless
		 * of which column triggered the render and regardless of which
		 * sizing path (linecount / multigraphs / per-service) the rest
		 * of this block takes. Without this, services that carry
		 * <!--DEVMON ...--> markers but neither provide a
		 * <!-- linecount= --> marker nor appear in the multigraphs list
		 * would never have their markers collected, and the
		 * marker-backed graphs would be silently dropped.
		 *
		 * devmongraphs_add_line() deduplicates by name, so the
		 * multigraphs per-line walk below is free to re-add the same
		 * markers without producing duplicates.
		 */
		devmon_markers_found = (devmongraphs_scan(&devmongraphs, restofmsg) > 0);
		...
		if (lcstr) {
			 ...
			 * If we already found embedded markers above, those *are* the evidence of graphs, so keep may_have_rrd. */
			if (devmongraphs_is_devmon_rrdname(rrd->xymonrrdname) && !devmon_markers_found) may_have_rrd = 0;
