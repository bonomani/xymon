	  ...
	  case AF_INET:
		{
			...
			if (includeport) sprintf(addrstring + strlen(addrstring), ":%d", ntohs(sin->sin_port));
		}
		break;
#endif
	  ...
	  case AF_INET6:
		{
			...
			if (includeport) {
				addrstring[0] = '[';
				inet_ntop(conn->family, conn->peer_sin, addrstring+1, sizeof(addrstring)-2);
				strcat(addrstring, "]");
 ...
 */
int conn_listen(int backlog, int maxlifetime,
		char *localaddr, enum sslhandling_t sslhandling,
		enum conn_cbresult_t (*usercallback)(tcpconn_t *, enum conn_callback_t, void *))
{
	...
	if (!local6) {
		struct in_addr addr;
		char *portdelim = strchr(localaddr, ':');
...
tcpconn_t *conn_accept(tcpconn_t *ls)
{
	...
	if (newconn->sock == -1) {
		...
		free(newconn);
		if ((errno != EAGAIN) && (errno != EWOULDBLOCK) && (errno != EINTR)) conn_info(funcid, INFO_WARN, "accept failed (%d: %s)\n", errno, strerror(errno));
		return NULL;
	}
...
#ifdef DEBUG_FOR_VALGRIND
			if (n > 0) VALGRIND_MAKE_MEM_DEFINED(buf, n);
#endif
		}
		else
...
/*
 * Setup a server instance, listening on the IP's and portnumber listed.
 ...
 * IP can be either IPv4 or IPv6, but must be numeric.
 *
...
char *conn_lookup_ip(char *hostname, int *portnumber, enum conn_ipproto_t preferredprotocol)
{
	...
	*addrstring = '\0';
	...
	switch (preferredprotocol) {
	  case CONN_IPPROTO_ANY: hints.ai_family = AF_UNSPEC; break;
	  case CONN_IPPROTO_V4:  hints.ai_family = AF_INET; break;
	  case CONN_IPPROTO_V6:  hints.ai_family = AF_INET6; break;
	}
	...
	res = getaddrinfo(hostname, portname, &hints, &addr);
