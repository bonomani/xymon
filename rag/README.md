...
`xymon_export.py` builds documents from one of three sources (`ingest.py <src>`):

- `status` — the live board via the `xymon` client (`xymondboard`).
- `history` — per-host flat-file history.
- `api` — the **canonical Xymon REST API** (the `feat/openapi` branch's `/states`),
  read over HTTP (`XYMON_RAG_API_URL`). Preferred: the API is the one place that
  turns Xymon into clean JSON for every consumer (RAG, n8n, dashboards), so the
  RAG consumes it rather than re-implementing a facade here.

```bash
XYMON_RAG_API_URL=http://localhost:8080/xymon/api/v1 python ingest.py api
```
