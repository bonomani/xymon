...
static inline const char **xymon_rrd_argv_cast(xymon_rrd_argv_item_t *argv)
{
	return (const char **)argv;
}
#else
typedef char *xymon_rrd_argv_item_t;
static inline char **xymon_rrd_argv_cast(xymon_rrd_argv_item_t *argv)
{
	return (char **)argv;
}
#endif
...
{
	return rrd_update(argc, xymon_rrd_argv_cast(argv));
}

static inline int xymon_rrd_create(int argc, xymon_rrd_argv_item_t *argv)
{
	return rrd_create(argc, xymon_rrd_argv_cast(argv));
}
...
{
	return rrd_fetch(argc, xymon_rrd_argv_cast(argv), start, end, step, dscount, dsnames, data);
}
				  ...
				  char ***calcpr, int *xsize, int *ysize, double *ymin, double *ymax)
{
	...
	return rrd_graph(argc, xymon_rrd_argv_cast(argv), calcpr, xsize, ysize, NULL, ymin, ymax);
#else
	(void)ymin;
	(void)ymax;
	return rrd_graph(argc, xymon_rrd_argv_cast(argv), calcpr, xsize, ysize);
#endif
}
