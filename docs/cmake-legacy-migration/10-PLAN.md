...
- [x] `docs/cmake-legacy-migration/refs/make_linux/server/inventory.tsv` is up to date with reference Makefiles.
...
- [x] Diff between `refs/make_linux/server/inventory.tsv` and the generated CMake list shows only documented exceptions.
- [x] Install hook is inline `install(CODE ...)` and uses `$ENV{DESTDIR}`.
- [x] `refs/make_linux/server/inventory.tsv` is versioned reference data and must only be updated when reference Makefiles change, with updates recorded in `STATUS-HISTORY.md`.
