...
int ntp_internal_probe(const char *ip, const char *srcip, int timeout, strbuffer_t *banner, double *offset_out)
{
	 ...
	 *
	 * Port 123 is hardcoded: the ntp test has no per-host port override, so
	 * keeping the signature ip-only loses nothing. */
	for (attempt = 0; attempt < tries; attempt++) {
		ntp_nonce(&xmt_sec, &xmt_frac);		/* unpredictable request nonce (anti-spoof) */
		ntp_build_request(req, xmt_sec, xmt_frac);
		n = udp_query_opt(ip, "123", req, sizeof(req), resp, sizeof(resp), per_try,
				  &qopts, &sent, &recv, &err);
		if (n >= 0) break;
	}
	if (n < 0) {
