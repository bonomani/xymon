...
- `ci/deps/` contains every dependency-related helper: the YAML-based package mappings (`ci/deps/data/deps-*.yaml`), the `packages-*.sh`/`packages-from-yaml.py` translators, installers (`install-*-packages.sh`), and the Python `check-deps.py` validator. This keeps all packaging logic in one place.
