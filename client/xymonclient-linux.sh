...
df_sentinel()
{
	...
	# and these are the processes that cannot be killed.
	#
	# Write the claim to a private file and hard-link it into place: link fails
	# if the target exists, so exactly one cycle wins, and what the winner
	# publishes is complete from the moment it is visible. "set -C" gave the
	# same exclusivity but not that: O_EXCL makes the *create* atomic, and a
	# reader between the create and the write got an empty string, which is
	# neither a claim nor a pid -- so it read the state as stale, removed the
	# files, and started a probe of its own on top of the first (@SoundGoof).
	# Winning also proves the directory is writable before any df starts.
	#
	# A subshell around the redirect: a redirection error on a special builtin
	# is fatal to the shell rather than a non-zero status. Under dash -
	# /bin/sh on Debian and Ubuntu - "return 124" would never run, the sentinel
	# would die with status 2, and run_df would emit nothing for the remote set
	# instead of the unavailable rows.
	#
