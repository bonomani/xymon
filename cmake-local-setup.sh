  ...
  case "${os_name}" in
    Linux)
      ...
      if ENABLE_LDAP="${enable_ldap}" VARIANT="${variant}" \
        bash "${root_dir}/scripts/ci/install-linux-packages.sh" --check-only --distro-family debian --distro ubuntu --version local; then
        echo "=== Install (Linux packages) ==="
        echo "All required packages already installed; skipping."
      else
        ENABLE_LDAP="${enable_ldap}" VARIANT="${variant}" \
        bash "${root_dir}/scripts/ci/install-linux-packages.sh" --install --distro-family debian --distro ubuntu --version local
      fi
      ;;
