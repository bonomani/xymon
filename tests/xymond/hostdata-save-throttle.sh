...
# Garbage and negative counts are rejected for the default of 5 (with atoi
# they would collapse to 0 and silently turn the module off).
saved=$(burst 8 junk)
[ "$saved" -eq 5 ] \
	|| fail "--recent-count=junk wrote $saved files, expected rejection and the default of 5"
saved=$(burst 8 -3)
[ "$saved" -eq 5 ] \
	|| fail "--recent-count=-3 wrote $saved files, expected rejection and the default of 5"
...
# Garbage and negative periods are rejected for the 60-minute default,
# inside which the burst is still throttled.
saved=$(burst 8 5 junk)
[ "$saved" -eq 5 ] \
	|| fail "--recent-period=junk wrote $saved files, expected rejection and a throttled burst of 5"
saved=$(burst 8 5 -1)
[ "$saved" -eq 5 ] \
	|| fail "--recent-period=-1 wrote $saved files, expected rejection and a throttled burst of 5"
