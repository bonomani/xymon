...
# Guarding only the disk report would move the hang rather than remove it: the
# disk df returns, and the client blocks in [inode] on the same dead mount.
# /usr/sysv/bin/df has no -T, so it is given the local mount points instead.
