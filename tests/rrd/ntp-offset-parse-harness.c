 ...
 * plumbing stubbed so the value that WOULD be written to ntpstat.rrd is captured.
 *
 ...
 * and that do_ntpstat parses "offset=" even at the very start of the message
 * (the #185 out-of-bounds-read case) without scaling its already-ms value.
 *
...
int main(void)
{
	...
	/* do_ntpstat parses "offset=" at the very start of msg (the #185 OOB case) and
	...
	do_ntpstat_rrd("host", "ntpstat", "", "", "offset=12.500000\n", (time_t)1000);
	{
		...
		check("do_ntpstat offset= at start (#185)", ms, 12.5);
	}
