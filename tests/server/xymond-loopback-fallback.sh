...
#
# xymond keeps a loopback listener so the client on the server keeps reporting
# when a physical interface goes away. That listener is an extra the daemon
# adds on its own, so it is best-effort: refusing to start because something
# else already holds 127.0.0.1 on that port would be a worse failure than
# starting without it. An address the operator actually asked for gets the
# opposite treatment -- coming up silently without it is the same class of
# fault as binding every interface, which is what this branch of the work
# exists to fix.
#
# That asymmetry is one `return` away from inverting in either direction, and
# both inversions are bad, so both directions are pinned here:
#
#   - a NAMED address that cannot bind must stop the daemon
#   - the LOOPBACK extra that cannot bind must not, and must say so
#
# The first needs nothing from the host and runs everywhere; the second needs
# a non-loopback address to name instead, and skips where there is none.
#
# The port is held by tests/lib/port-blocker.c, which binds without listening.
# That reserves it on the BSDs as well as on Linux: SO_REUSEADDR permits a
# wildcard and a specific bind to overlap, but not two binds of the same
# address and port, so the collision below is deterministic on both.
#
