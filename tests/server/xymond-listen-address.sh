...
#
# The bug this pins was invisible on Linux for eleven years. --listen=IP:PORT
# split the string but put the colon back, so inet_aton() was handed "IP:PORT",
# failed, and left the zeroed sin_addr behind -- and the daemon bound every
# interface for anyone who asked for one address. Nothing said so.
#
# It was invisible to the suite too. Every other test that starts a daemon
# passes --listen=127.0.0.1:PORT and then talks to 127.0.0.1, which a wildcard
# bind answers perfectly well; the only test that noticed was the port-retry
# one, and only on the BSDs, where a wildcard and a specific bind do not
# collide. Upstream CI runs neither. So the fix arrived with no test that
# would fail if it were reverted -- which is what this file is for.
#
# Two questions, because the bug was two mistakes:
#
#   - an address that cannot be parsed must stop the daemon, not silently
#     become the wildcard. This is the half that needs nothing from the host,
#     so it runs everywhere and catches a straight revert on its own.
#   - an address that can be parsed must be the only one bound. Asked by
#     probing a non-loopback address of this host, so it needs one to exist.
#
