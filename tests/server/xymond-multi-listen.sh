...
#
# xymond listens on every address it is given, and always keeps a loopback
# listener unless told not to.
#
# One address is not enough for a server: the operator wants a chosen interface
# AND the loopback, so the client on the server itself keeps reporting when the
# physical interface goes away. Before --listen was fixed that came free by
# accident, because every --listen=IP:PORT bound the wildcard; now that it
# binds what it is told, the set has to be expressible.
#
# Pinned here, by asking a running daemon on each address in turn:
#
#   - a comma-separated list binds every entry, each with its own port
#   - a single address still behaves as it always did
#   - naming only non-loopback addresses still leaves 127.0.0.1 answering
#   - 0.0.0.0 already covers loopback, so no second socket is attempted
#   - --no-loopback is the way to say you really mean one address
#   - SIGHUP adds and retires addresses without touching the loopback listener,
#     and a bad address in the new set leaves the old one serving
#
